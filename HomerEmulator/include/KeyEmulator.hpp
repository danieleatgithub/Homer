...
#define KEY_EMULATOR_DIE(str, args...) do { \
        Logger logdie = Logger::getInstance(LOGEMULATOR); \
        LOG4CPLUS_ERROR(logdie, str); \
        perror(str); \
        exit(EXIT_FAILURE); \
    } while(0)
...
class KeyEmulator {
 ...
 private:
   ...
   */
  void init(const char *name, __u16 type = BUS_USB, __u16 vendor = 0xc0da,
            __u16 product = 0xcafe, __u16 version = 1,
            unsigned int key_us_auto_release = 100000) {
    DIR *pdir;
    struct dirent *pent;
    this->running = false;
    ...
    // Die on error
    if (fd < 0)
      KEY_EMULATOR_DIE("error: open");
    if (ioctl(fd, UI_SET_EVBIT, EV_KEY) < 0)
      KEY_EMULATOR_DIE("error: ioctl EV_KEY");
    if (ioctl(fd, UI_SET_EVBIT, EV_SYN) < 0)
      KEY_EMULATOR_DIE("error: ioctl EV_SYN");
    if (ioctl(fd, UI_SET_KEYBIT, KEY_ENTER) < 0)
      KEY_EMULATOR_DIE("error: ioctl KEY_ENTER");
    if (ioctl(fd, UI_SET_KEYBIT, KEY_UP) < 0)
      KEY_EMULATOR_DIE("error: ioctl KEY_UP");
    if (ioctl(fd, UI_SET_KEYBIT, KEY_DOWN) < 0)
      KEY_EMULATOR_DIE("error: ioctl KEY_DOWN");
    if (ioctl(fd, UI_SET_KEYBIT, KEY_LEFT) < 0)
      KEY_EMULATOR_DIE("error: ioctl KEY_LEFT");
    if (ioctl(fd, UI_SET_KEYBIT, KEY_RIGHT) < 0)
      KEY_EMULATOR_DIE("error: ioctl KEY_RIGHT");
    ...
    if (event.empty())
      KEY_EMULATOR_DIE("syspath event not found");
    (void) closedir(pdir);
  }
   ...
   */
  void k_write(__u16 type, __u16 code, __s32 value,
               unsigned int event_us_pause = 100) {
  ...
  __attribute__((deprecated)) void key_thread_emulator() {
    ...
    tv.tv_usec = 0;
    if (running)
      KEY_EMULATOR_DIE("KeyEmulator Already running");
    running = true;
    LOG4CPLUS_INFO(
        logemu,
        "KeyEmulator thread STARTED NAV=8624 '-'=auto release toggle '+'=send release 'q'=Thread quit");
        ...
        switch (ch) {
          ...
          case '-':
            ...
            if (key_us_auto_release == 0) {
              ...
              LOG4CPLUS_INFO(
                  logemu,
                  "KeyEmulator: auto release active (" << key_us_auto_release
                      << ")");
            } else {
              ...
              LOG4CPLUS_INFO(
                  logemu,
                  "KeyEmulator: auto release deactivated ("
                      << key_us_auto_release << ")");
            }
            break;
          case 'q':
    ...
    LOG4CPLUS_INFO(logemu, "KeyEmulator thread terminated");
  }
 ...
 public:
  ...
  KeyEmulator(const char *name, __u16 type, __u16 vendor, __u16 product,
              __u16 version, unsigned int key_us_auto_release) {
    init(name, type, vendor, product, version, key_us_auto_release);
  }
  ~KeyEmulator() {
//    	if(running) stop();
    if (ioctl(fd, UI_DEV_DESTROY) < 0)
      KEY_EMULATOR_DIE("error: ioctl in ~KeyEmulator");
    close(fd);
  }
  void gl_key_event(unsigned char key, int x, int y, __s32 value) {
    ...
    switch (key) {
      ...
      case 'q':
        ...
        break;
      default:
        LOG4CPLUS_ERROR(logemu, " Unmanaged key:" << key);
        current_key = KEY_ESC;
    }
    // Write key events on the device
...
#pragma GCC diagnostic pop
  ...
  __attribute__((deprecated)) bool isRunning() const {
    return (running);
  }
  ...
  __attribute__((deprecated)) void setKeyUsAutoRelease(
      unsigned int keyUsAutoRelease) {
