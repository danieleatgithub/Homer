...
class WinstarEmulator : public Winstar {
 ...
 public:
  ...
  void i2cbus_register() {
    board.getEmulatedI2c0().reg_write(
        ...
        LOG4CPLUS_TRACE(logemu, "SZ=" << size << " p[0]=" << hex << p[0]);
      });
  }
  void backLight_register() {
    board.getEmulatedLcdBacklight().reg_write(
        light_reg, [&] ( int fd, const void *buffer, size_t size ) {
          const unsigned char *p = (const unsigned char *)buffer;
          Logger logemu = Logger::getInstance(LOGEMULATOR);
          LOG4CPLUS_TRACE(logemu, "SZ=" << size << " p[0]=" << hex << p[0]);
          if(p[0] == 0x30) {
